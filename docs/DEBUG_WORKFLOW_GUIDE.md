...
Claude가 `data_get_study_series`로 봉별 마스크를 뽑아 0인 비트를 찾고,
...
| `/call` | POST | `{ "tool": "...", "params": { ... } }` → 툴 결과 JSON |
...
| `data_get_study_series`가 "No study columns matched" | `study_filter` 오타(대소문자는 무시됨) 또는 전략이 차트에 없음. 첫 사용 시 `ui_evaluate`로 exportData 스키마 필드명 확인 (스킬 Caveats 참조) |
